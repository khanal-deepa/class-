...
class Model:
    ...
    #     total_samples = len(train_inputs)  # Assuming train_inputs is your entire training dataset
    #     # Inside the training loop
    #     for epoch in range(self.epochs):
    #         print(f"Epoch {epoch + 1}/{self.epochs}")
    #         for sample_idx in range(0, total_samples, batch_size):
    #             batch_inputs = train_inputs[sample_idx: sample_idx + batch_size]
    #             batch_outputs = train_outputs[sample_idx: sample_idx + batch_size]
                
    #             # Calculate training percentage completion
    #             samples_processed = sample_idx + len(batch_inputs)
    #             training_percentage = (samples_processed / total_samples) * 100
...
#                print(time2 - time1)
    ...
    #     # Gather model configuration/parameters that need to be saved
    #     model_data = {
    #         'last_layer': self.last_layer,  # Example attribute to save
    #         'epochs': self.epochs,
    #         'learning_rate': self.learning_rate,
    #         # Add more attributes here that you want to save
    #     }
    ...
    #     # Load the saved model using Keras load_model
    #     self.last_layer = load_model(filepath)
    #     self.network_layers = self.get_layers()  #